...
case $os_type in
    ...
    MINGW*|MSYS*|CYGWIN*)
        os_name="windows"
        target_dir="/c/wait"  # Use C:\wait on Windows
        ;;
    *)
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --install*)
            ...
            ;;
        *)
            # Non-install arguments go to wait_args
...
# Remove leading space from wait_args if present
wait_args="${wait_args# }"

wait_agent_path="${target_dir}/agent"
...
echo -e "${WHITE}===========================================${NC}"
echo -e "${WHITE}    wait agent Installation Script     ${NC}"
echo -e "${WHITE}===========================================${NC}"
echo ""
...
log_config "  Release source: ${GREEN}${release_repo_url}${NC}"
log_config "  Binary arguments: ${GREEN}$wait_args${NC}"
...
uninstall_previous() {
    ...
    if command -v systemctl >/dev/null 2>&1 && systemctl list-unit-files | grep -q "${service_name}.service"; then
        ...
        system_plist="/Library/LaunchDaemons/com.wait.${service_name}.plist"
        user_plist="$HOME/Library/LaunchAgents/com.wait.${service_name}.plist"
    ...
    if [ -f "$wait_agent_path" ]; then
        log_info "Removing old binary..."
        rm -f "$wait_agent_path"
    fi
}
...
file_name="wait-agent-${os_name}-${arch}"
...
if [ -n "$github_proxy" ]; then
    # Use proxy for release downloads
    download_url="${github_proxy}/${release_repo_url}/${download_path}/${file_name}"
else
    # Direct access to release downloads
    download_url="${release_repo_url}/${download_path}/${file_name}"
fi
...
if ! curl -L -o "$wait_agent_path" "$download_url"; then
    log_error "Download failed"
    exit 1
fi

# Set executable permissions
chmod +x "$wait_agent_path"
log_success "wait-agent installed to ${GREEN}$wait_agent_path${NC}"
...
if [ "$init_system" = "nixos" ]; then
    ...
    echo -e "${CYAN}  description = \"wait agent Service\";${NC}"
    ...
    echo -e "${CYAN}    ExecStart = \"${wait_agent_path} ${wait_args}\";${NC}"
...
name="wait agent Service"
description="wait monitoring agent"
command="${wait_agent_path}"
command_args="${wait_args}"
command_user="root"
...
[Service]
Type=simple
ExecStart=${wait_agent_path} ${wait_args}
...
EOF
    ...
    if [[ "$target_dir" =~ ^/Users/.* ]] || [ "$EUID" -ne 0 ]; then
        ...
        plist_file="$plist_dir/com.wait.${service_name}.plist"
...
<dict>
    <key>Label</key>
    <string>com.wait.${service_name}</string>
    <key>ProgramArguments</key>
    <array>
        <string>${wait_agent_path}</string>
EOF
    
    # Add program arguments if provided
    if [ -n "$wait_args" ]; then
        echo "$wait_args" | xargs -n1 printf "        <string>%s</string>\n" >> "$plist_file"
    fi
...
pre-start script
    test -x ${wait_agent_path} || { stop; exit 0; }
end script

# Start
script
    exec ${wait_agent_path} ${wait_args}
end script
EOF
    # enable Upstart unit
...
echo -e "${WHITE}===========================================${NC}"
if [ -f /etc/NIXOS ]; then
    log_success "wait-agent binary installed!"
    ...
    log_success "wait-agent installation completed!"
fi
log_config "Service: ${GREEN}$service_name${NC}"
log_config "Arguments: ${GREEN}$wait_args${NC}"
echo -e "${WHITE}===========================================${NC}"
